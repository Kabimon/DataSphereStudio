<template>
  <div class="page-bgc">
        ...
        <div class="app-list-main">
          <div class="app-list-tabs">
            <Tabs v-model="showTab">
              <Tab-pane
                name="dev"
                :label="$t('message.workspace.DSSDEV')"
              ></Tab-pane>
              <Tab-pane
                name="shop"
                :label="$t('message.workspace.DSSSHOP')"
              ></Tab-pane>
            </Tabs>
            ...
            <div v-show="showTab === 'shop'">
              <div class="app-shop-top">
                ...
                <div class="app-list">
                  <div class="nodata-tips" v-if="favoriteApps.length === 0">
                    {{ $t("message.workspace.home.tips") }}
                  </div>
                  <div
                    v-else
                    v-for="(item, index) in favoriteApps"
                    :key="item.title"
                    class="app-item-wrap"
                    :class="{ shadow: setting }"
                    @click="setting ? null : linkTo(item, item.url)"
                  >
                    <div
                      v-if="setting"
                      class="close-wrap"
                      @click.stop="deleteFavoriteApp(item.id, index)"
                    >
                      <Icon type="md-close" size="20"></Icon>
                    </div>
                    <SvgIcon
                      class="app-icon"
                      :icon-class="item.icon ? item.icon : 'shujukaifa-logo'"
                    />
                    <span class="label">{{
    ...
    <Modal v-model="show" :title="$t('message.workspace.home.dlgTitle')">
      <Form
        ref="dynamicForm"
        :model="formDynamic"
        :label-width="100"
      ...
      >
        <FormItem
          :label="$t('message.workspace.home.selectType')"
          prop="selectType"
        >
        ...
        <FormItem
          :label="$t('message.workspace.home.selectApp')"
          prop="selectApp"
        >
      ...
      <div slot="footer">
        <Button @click="show = false">{{
          $t("message.workspace.home.cancel")
        }}</Button>
        <Button type="primary" @click="addFavoriteApp">{{
          $t("message.workspace.home.save")
        }}</Button>
      </div>
    </Modal>
    ...
    return {
      ...
      rules: {
        selectApp: [
          {
            required: true,
            message: this.$t("message.workspace.home.selectApp"),
            trigger: "change",
          },
        ],
        selectType: [
          {
            required: true,
            message: this.$t("message.workspace.home.selectType"),
            trigger: "change",
          },
        ],
      },
  ...
  computed: {
    ...
    apps() {
      if (this.formDynamic.selectType) {
        const result = this.applications.find(
          (item) => item.id == this.formDynamic.selectType
        );
        result.appInstances.map((item) => {
          item.had = false;
          if (
            this.favoriteApps.find(
              (fItem) => item.id === fItem.menuApplicationId
            )
          ) {
            item.had = true;
          }
  ...
  methods: {
    ...
    init() {
      this.$router.app.$on("getChangeCookies", () => {
        ...
        api
          .fetch(
            `${this.$API_PATH.WORKSPACE_PATH}workspaces/${this.workspaceId}/favorites`,
            "get"
          )
          .then((data) => {
    ...
    deleteFavoriteApp(favoriteId, index) {
      api
        .fetch(
          `${this.$API_PATH.WORKSPACE_PATH}workspaces/${this.workspaceId}/favorites/${favoriteId}`,
          {},
          { method: "delete", data: {} }
        )
        .then(() => {
        ...
        if (valid) {
          ...
          if (favoriteApp) {
            return this.$Message.error(
              `${this.$t("message.workspace.home.repeat")}`
            );
          }
          this.show = false;
          api
            ...
            .then((data) => {
              const app = this.findAppByApplicationId(
                this.formDynamic.selectApp
              );
    ...
    getSideDatas() {
      api
        .fetch(
          `${this.$API_PATH.WORKSPACE_PATH}getSideInfos`,
          {
            workspaceID: 106,
          },
          "get"
