...
export default {
  ...
  watch: {
    ...
    "$route.query.projectID"() {
      ...
      this.getAreaMap();
      this.getProjectData();
      if (this.topTabList[1])
        this.topTabList[1].name = this.$route.query.projectName;
    },
  ...
  computed: {
    srcUrl() {
      let url = "";
      const found = this.selectDevprocess.find(
        (item) => this.modeOfKey === item.dicValue
      );
      if (found) {
        const { projectName, projectId } = this.$route.query;
        const workspaceId = this.getCurrentWorkspaceId();
        const workspaceName = this.getCurrentWorkspaceName();
        url = util.replaceHolder(found.url, {
          workspaceId,
          workspaceName,
          projectId,
          projectName,
  ...
  methods: {
          ...
          if (openOrchestrator) {
            ...
            const isIn = this.tabList.find(
              (item) =>
                ...
                item.latestVersion === params.latestVersion
            );
            if (!isIn || isIn === -1) {
              // 限制打开tab数量
              if (this.tabList.length >= 10)
                return this.$Message.warning("最做只可打开十个tab");
              this.current = { ...params };
    ...
    publishSuccess(currentOrchetratorData) {
      this.tabList = this.tabList.filter(
        (i) =>
          i.tabId !==
          String(currentOrchetratorData.orchestratorId) +
            currentOrchetratorData.orchestratorVersionId
      );
    },
  },
};
