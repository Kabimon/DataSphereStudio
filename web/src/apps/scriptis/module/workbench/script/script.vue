  ...
  computed: {
    ...
    bottomTab() {
      ...
      return {
        ...
        result:
          this.scriptResult.total ||
          (this.script.resultList && this.script.resultList.length),
    ...
    if (this.work.data) {
      ...
      this.work.data = this.script = new Script(
        Object.assign({}, supportedMode, {
          ...
          executable: !supportedMode.executable
          ...
          this.$Notice.info({
            ...
            render: (h) => {
              return h(
                ...
                `${this.$t("message.scripts.notice.sendStart.render")} ${
                  this.work.filename
                } ！`
              );
            },
          ...
          this.$Notice.success({
            ...
            render: (h) => {
              return h(
                ...
                `${this.work.filename} ${type}${this.$root.$t(
                  "message.scripts.notice.querySuccess.render"
                )}：${costTime}！`
              );
            },
            name,
        ...
        api
          ...
          .then(() => {
            ...
            this.$Notice.warning({
              ...
              desc: `${this.$t("message.scripts.notice.kill.desc")} ${
                this.work.filename
              } ！`,
              name: name,
              duration: 3,
                  ...
                  api
                    ...
                    .then(() => {
                      ...
                      this.$Message.success(
                        this.$t("message.scripts.constants.success.save")
                      );
                      // 提交最新的内容，更新script.data和script.oldData
                      this.script.oldData = this.script.data;
    ...
    async initHistory() {
      try {
        const rst = await api.fetch(
          `/jobhistory/${this.work.taskID}/get`,
          "get"
        );
        if (rst) {
          const option = rst.task;
          const supportList = this.getSupportModes();
          const supportedMode = find(supportList, (p) =>
            p.rule.test(this.work.filename)
          );
          this.work.data = this.script = new Script(
            Object.assign({}, supportedMode, option, {
              fileName: this.work.filename,
