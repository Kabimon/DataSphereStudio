<template>
  <div class="statistics">
    <Split v-model="split" :style="{ height: panelHeight + 'px' }">
        ...
        <p>
          ...
          >
            <span
              ...
              :style="{ width: enEnv ? '140px' : '100px' }"
            >{{ item.title }}:
            </span>
            <span
              class="statistics-card-item-value"
              :style="{
                width: enEnv ? 'calc(100% - 144px)' : 'calc(100% - 104px)',
              }"
      ...
      <Card :bordered="false" class="statistics-card" slot="right">
        <p slot="title">{{ $t("message.scripts.tableDetails.FQXX") }}</p>
        <span v-show="statisticInfo.partitions" class="statistics-card-label"
        >{{ `* ${$t("message.scripts.tableDetails.DJFQCK")}` }}
          <Icon
            @click="Partitionsort"
            type="md-swap"
            style="transform: rotate(90deg); margin-left: 21px"
          />
        </span>
        <Tree
          ref="partTree"
...
export default {
  ...
  methods: {
    ...
    async getCurrentNode(node) {
      ...
      if (nodePartition) {
        const str =
          !isEmpty(partNode) &&
          moment
            .unix(nodePartition.modificationTime / 1000)
            .format("YYYY-MM-DD HH:mm:ss");
        const isFirstLevel = !isEmpty(partNode);
        const isHasSize =
          !isEmpty(partNode) &&
          partNode.title.indexOf(this.$t("message.scripts.tableDetails.DX")) ===
            -1;
        if (partNode && isFirstLevel && isHasSize && str) {
          // \xa0代表空格
          partNode.title += `\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0（${this.$t(
            "message.scripts.tableDetails.FQDX"
          )}${nodePartition.partitionSize}，${this.$t(
            "message.scripts.tableDetails.WJS"
          )}${nodePartition.fileNum}，创建时间：${str}）`;
    ...
    getChildrenPartition(node) {
      ...
      return api
        .fetch("datasource/getPartitionStatisticInfo", params, "get")
        .then((res) => {
